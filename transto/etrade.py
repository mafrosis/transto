...
def load_csvs(vestfile: str, sellfile: str) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
...
def vesting(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    df_grants = df_grants.join(
        dfv[dfv['Vest Period'] == 1].set_index('Grant Number')['Vest Date'], on='Grant Number'
    )
    ...
    df_unvested = df_unvested[(df_unvested['Vest Year'] >= datetime.datetime.now().year-1) & (df_unvested['Vest Year'] <= datetime.datetime.now().year+2)]
    ...
    df_vest_schedule = (
        df_unvested
            .groupby(['Vest Year', 'Grant Number'])
            .agg({'Vest Qty': 'sum'})
            .reset_index()
    ...
    for i, year in enumerate(range(datetime.datetime.now().year-1, datetime.datetime.now().year+3)):
        col_num = (i * 2) + 2
